...
## 🆕 New: MCPPlus Post-Processing Extension

**Reduce LLM token costs by up to 70%** with intelligent tool output filtering! MCPPlus transparently post-processes MCP tool outputs, keeping only the information your agent needs.

✨ **Key Benefits:**
- 💰 **50-70% cost reduction** - Dramatically lower token usage
- 🎯 **Dual extraction** - Direct text extraction + code-based filtering
- 🔌 **Drop-in integration** - Transparent wrapper for MCP clients
- 📊 **Benchmarking ready** - Full integration with MCP-Universe benchmarks

[→ Learn more about MCPPlus](mcpuniverse/extensions/mcpplus/README.md)

---

## What is MCP-Universe?

MCP-Universe is a **comprehensive ecosystem** for MCP (Model Context Protocol) development, testing, and optimization. Whether you're benchmarking AI agents, optimizing token costs, or building production MCP applications, MCP-Universe provides the tools you need.
...
|---------|-------------|--------|
| **🧪 Benchmarking** | Evaluate LLMs and agents across real-world MCP tasks | ✅ Available |
| **🔧 MCPPlus** | Intelligent post-processing for cost reduction | ✅ Available |
| **🎓 Training** | Fine-tune agents for MCP interactions | 🚧 Coming Soon |
| **🏗️ Workflows** | Orchestrate multi-agent MCP applications | ✅ Available |
...
- [Benchmarking LLMs and Agents](#benchmarking-llms-and-agents)
...
- [MCPPlus: Cost Optimization](#mcpplus-cost-optimization)
...
- **Extensions** (`mcpuniverse/extensions/`): Advanced features like MCPPlus
...
│                        Agent Layer                              │
├─────────────────────────────────────────────────────────────────┤
│  BasicAgent │   ReActAgent    │  FunctionCall  │   Extensions   │
│             │                 │     Agent      │    (MCPPlus)   │
└─────────────┬─────────────────┬────────────────┬────────────────┘
              │                 │                │
┌─────────────▼─────────────────▼────────────────▼────────────────┐
...
## MCPPlus: Cost Optimization

**MCPPlus** is a post-processing extension that reduces LLM token costs by intelligently filtering tool outputs. It's designed to transparently wrap MCP clients and work seamlessly with the MCP-Universe benchmarking framework.

### Key Features

- **💰 50-70% Cost Reduction** - Dramatically reduce token usage while maintaining quality
- **🎯 Dual Post-Processing** - Generates both direct extraction and code-based filtering in a single LLM call
- **🔌 Drop-in Integration** - Transparent wrapper for MCP clients, no agent code changes needed
- **📊 Benchmark Integration** - Full support for MCP-Universe benchmark tracking

### Quick Start with MCPPlus
...
# Initialize manager with wrapper
manager = MCPWrapperManager(
    config="configs/server_list.json",
...
# Build wrapped client - post-processing happens automatically!
client = await manager.build_client("your-mcp-server")
```

### How It Works

MCPPlus adds an `expected_info` parameter to all tool calls. The agent specifies what information it needs, and MCPPlus:

1. **Checks token threshold** - Only processes outputs above threshold
2. **Dual extraction** - Generates both direct text extraction and Python code
3. **Executes safely** - Runs code in sandboxed environment
4. **Returns both** - Provides both extraction methods for robustness

**Result:** Your agent receives concise, relevant information instead of verbose raw outputs.

### Documentation

For comprehensive MCPPlus documentation, examples, and API reference, see:
- [MCPPlus Documentation](mcpuniverse/extensions/mcpplus/README.md)
- [MCPPlus Examples](mcpuniverse/extensions/mcpplus/examples/)
